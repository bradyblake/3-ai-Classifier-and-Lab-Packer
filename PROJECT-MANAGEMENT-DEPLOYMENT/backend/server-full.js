...
app.post('/api/analyze-live', upload.single('pdf'), async (req, res) => {
  ...
  try {
    ...
    // Apply ENHANCED customSDSEngine for structured data extraction
    console.log('🧪 Using ENHANCED customSDSEngine for data extraction...');
    const { customSDSEngine } = await import('./services/customSDSEngine.js');
    const structuredData = customSDSEngine.extract(extractionResult.text);
    console.log('🧪 Enhanced extraction complete - Product:', structuredData.productName);
    console.log('🧪 Enhanced extraction complete - Physical State:', structuredData.physicalState);
    console.log('🧪 Enhanced extraction complete - Composition items:', structuredData.composition?.length || 0);
    ...
    const extractedData = {
      ...
      properShippingName: structuredData.dotShippingName
    };
    ...
    const result = {
      ...
      // Classification data at top - map deterministic classifier fields to frontend expected fields
      classification: {
        success: true,
        method: 'PyMuPDF-Live',
        processingTime: `${processingTime}ms`,
        
        // Map deterministic classifier fields to frontend expected fields
        productName: classification.productName,
        manufacturer: structuredData.manufacturer || 'Unknown',
        flashPoint: classification.flashPoint?.celsius || null,
        pH: classification.pH,
        physicalState: classification.physicalState,
        density: structuredData.density || null,
        
        // Additional structured data from JSON extractor
        productCode: structuredData.productCode,
        revision: structuredData.revision,
        ...
        composition: structuredData.composition || [],
        chemicals: structuredData.composition || [], // Alias for frontend compatibility
        ...
        state_classification: classification.state_classification,
        state_form_code: classification.state_form_code,
        state_codes: classification.state_codes,
        
        // DOT classification
        unNumber: classification.unNumber,
        properShippingName: classification.properShippingName,
        hazardClass: extractedData.hazardClass || 'N/A',
        packingGroup: extractedData.packingGroup || 'N/A',
        
        // Additional data
        chemicals: extractedData.chemicals || [],
        confidence: 95,
        aiProvider: 'deterministic',
        reasoning: classification.reasoning,
        
        // Include original classification for debugging
        originalClassification: classification,
        extractedData: extractedData,
        structuredExtraction: structuredData // Full JSON extraction result
      },
