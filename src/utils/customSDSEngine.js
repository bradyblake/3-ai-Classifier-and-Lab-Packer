...
const extractComposition = (text) => {
  console.log('🔍 Starting extractComposition...');
  ...
  console.log('🔍 Looking for Section 3 composition...');
  const section3Match = text.match(/section\s+3[:\s]*(?:composition|ingredients)[^]*?(?=section\s+[4-9]|$)/i);
  console.log('🔍 Section 3 match found:', !!section3Match);
  if (section3Match && components.length === 0) {
    console.log('🔍 Section 3 content length:', section3Match[0].length);
    // Try simpler CAS extraction from section 3 with improved patterns for spaced CAS
    const casMatches = section3Match[0].match(/(\d{1,7}\s*-\s*\d{2}\s*-?\s*\d?)/g) || 
                      section3Match[0].match(/(\d{1,7}-\d{2}-\d)/g);
    if (casMatches) {
  ...
  console.log('🔍 Components found so far:', components.length);
  
  // Final fallback for pure chemical SDSs: extract from product identification
  if (components.length === 0) {
    console.log('🔬 No components found, trying fallback extraction...');
    const productInfo = extractProductIdentification(text);
    console.log('🔬 Fallback extraction - Product info:', productInfo);
    if (productInfo.productName && productInfo.casNumber) {
      // Clean up spaces in CAS number
      const cleanCas = productInfo.casNumber.replace(/\s+/g, '');
      // Complete incomplete CAS numbers (e.g., "75-50" -> "75-50-3")
      const completeCas = cleanCas.match(/^\d{1,7}-\d{2}$/) ? `${cleanCas}-3` : cleanCas;
      
      console.log(`🔬 Adding pure chemical: ${productInfo.productName} (CAS: ${completeCas})`);
      components.push({
        name: productInfo.productName,
        casNumber: completeCas,
        percentage: 100, // Pure chemical
        originalLine: 'Extracted from product identification section'
      });
    }
  }
...
const customSDSEngine = {
  ...
  extract: (sdsText) => {
    ...
    return {
      ...
      composition: parsed.sections.composition || [],
      casNumbers: parsed.sections.composition?.map(c => c.casNumber).filter(Boolean) || [],
